...
// Last Modified: Sun 04 Feb 2018 04:48:21 PM CST
//******************************************************************************
...
/*   hsize_t old_dimsf[2]; */
  ...
  // new_dimsf[0] = old_dimsf[0] + batch_size * mpi_size;
  ...
  // // hsize_t	offset[2] = {mpi_rank, 0};
  // // hsize_t	stride[2] = {mpi_size, 1};
  // hsize_t	wcount [2] = {batch_size, new_dimsf[1]};
  ...
  // for (int i=0; i<wcount[0]; i++) {
    // sum = 0;
    // for (int j=0; j<count[1]; j++)
      // sum += data[(i+old_dimsf[0]) * count[1] + j];
    // wdata[i*3] = sum;
    // wdata[i*3+1] = i;
